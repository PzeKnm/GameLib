...
namespace RaspberryGPIOManager
{
  public class GPIOPinDriver : IDisposable
  {
    ...
    public Pin GPIOPin
    {
      ...
      private set
      {
        lock (_exported_pins)
        {
          ...
          {   
            if(IsPinExported(value))
            {
              WriteToPi(GPIO_ROOT_DIR + "unexport", value.ToString().Substring(4));
            }

            WriteToPi(GPIO_ROOT_DIR + "export", value.ToString().Substring(4));      
    ...
    private bool IsPinExported(Pin GPIOPin)
    {
      String sPath = String.Format("{0}gpio{1}", GPIO_ROOT_DIR, GPIOPin.ToString().Substring(4));
    ...
    public GPIODirection Direction
    {
      ...
      set
      {
        ...
        {
          WriteToPi(String.Format("{0}gpio{1}/direction", GPIO_ROOT_DIR, GPIOPin.ToString().Substring(4)), (value == GPIODirection.In ? "in" : "out"));
    ...
    public GPIOState State
    {
      ...
      set
      {
        ...
        {
            WriteToPi(String.Format("{0}gpio{1}/value", GPIO_ROOT_DIR, GPIOPin.ToString().Substring(4)), (value == GPIOState.High ? "1" : "0"));
        }
      }
    }
    ...
    public GPIOPinDriver(Pin gpioPin, bool _bSimMode)
        : this(gpioPin, GPIODirection.Out, GPIOState.Low, _bSimMode) { }
